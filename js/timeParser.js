...
function parseSlot(timeRaw) {
    ...
    let m = timeRaw.match(/([一二三四五六日])[,，]?(\d{3,4})[~\-－]([一二三四五六日]?)[,，]?(\d{3,4})/);
    if (m) {
        const day = m[1];
        ...
        const first = slots[0].code;
        const last = slots[slots.length - 1].code;
        return `星期${day} 第 ${first}-${last} 節`;
    }
...
function parseTimeToSchedule(timeRaw) {
    ...
    if (m) {
        ...
        const slots = [];
        const allSlotCodes = SLOT_TABLE.map(s => s.code);
        const startIdx = allSlotCodes.indexOf(startSlot);
        const endIdx = allSlotCodes.indexOf(endSlot);
        
        if (startIdx !== -1 && endIdx !== -1) {
            for (let i = startIdx; i <= endIdx; i++) {
                slots.push(allSlotCodes[i]);
            }
        }
        return { day, slots };
    }
